 ...
 * `prefers-reduced-motion` by skipping animations outright, and under reduced motion this cross-fade
 * is the *whole* transition (STYLE.md §3) — a skipped one would leave items popping in and out. And
 * the canvas renders on demand, so something outside the render pass has to ask for the next frame:
 * `invalidate()` called from inside `useFrame` is folded into the frame already being drawn.
 */
...
/** Wires a clock-driven opacity fade for everything under `target` and returns its trigger. */
export function useMaterialFade(target: RefObject<Group | null>): RunFade {
  ...
  return (from, to, ms) => {
    ...
    const tick = (): void => {
      const progress = ms <= 0 ? 1 : Math.min(1, (performance.now() - startedAt) / ms);
      applyFade(current, target.current, from + (to - from) * progress);
      invalidate();
      current.frame = progress < 1 ? requestAnimationFrame(tick) : 0;
    };
