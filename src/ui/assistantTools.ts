...
import { waitForToolsReady } from "../tools/readiness";
 ...
 *
 * The fallback assistant re-reads the dynamic tool list between rounds. The registry now owns the
 * readiness check, including asynchronous host registration, so this is only a bounded adapter for
 * the assistant loop. Returns immediately when no group change is pending.
 */
export async function assistantToolsSettled(timeoutMs = SETTLE_TIMEOUT_MS): Promise<void> {
  const registry = liveRegistry();
  if (!registry) return;
  await waitForToolsReady(() => registry.settled(), timeoutMs);
}
